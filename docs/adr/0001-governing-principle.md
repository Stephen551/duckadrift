...
This is the Pact. If a build decision violates this sentence, the decision is wrong even if it works.

## Consequences

What this sentence rules out — the test of a real principle:
...
- Rules out confidence thresholds as constants in code. Calibrated confidence → thresholds are measured artifacts produced by the calibration harness, recalibrated per model version. A number typed by a human into a config default is a provisional lie to be replaced by evidence.
...
- Tier 0 checks (D1–D7) are the only checks permitted to fail CI, and only because they are provably false-positive-free by construction.
- Tier 1 checks (S1–S5) are annex-only until a calibration entry exists for the exact `{backend, model, effort}` tuple in use; an uncalibrated run is loudly labeled `UNCALIBRATED`, never silently conservative.
- Channel assignment inverts the naive design: `critical`-severity ADRs *lower* the confidence bar to interrupt, because the cost of a miss buys tolerance for a false alarm — declared per-decision in frontmatter, not assumed globally.
- Silence is itself a violation: quota exhaustion, skipped coverage, and fork-PR credential gaps must be reported loudly, never absorbed quietly.
